...
public class ItemBookingService {
    ...
    public ItemBookingService(@Qualifier("DbItemRepo") ItemRepository itemRepository, ItemMapper itemMapper,
                              BookingService bookingService, CommentRepository commentRepository, CommentMapper commentMapper) {
    ...
    public List<ItemWithCommentAndBookingDto> getAllUserItems(long userId) {
        return itemRepository.findByOwnerId(userId).stream().map(i -> {
            ...
            List<CommentResponseDto> comments = commentRepository.findByItemId(i.getId())
                    .stream().map(commentMapper::mapCommentToResponse).toList();
